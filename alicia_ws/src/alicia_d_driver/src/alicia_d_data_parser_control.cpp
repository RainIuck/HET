...
void AliciaDDataParserControl::parse_joint_data(const std::vector<uint8_t>& frame)
{
    ...
    std::vector<uint16_t> joint_raw_values(6, 0);
    ...
    if (debug_mode_) {
        static int joint_debug_count = 0;
        std::vector<double> degrees;
        for (double rad : joint_values) {
            degrees.push_back(rad * 180.0 / M_PI);
        }
        if ((joint_debug_count++ % 20) == 0) {
            std::ostringstream frame_hex;
            for (uint8_t byte : frame) {
                frame_hex << std::uppercase << std::hex << std::setw(2) << std::setfill('0')
                          << static_cast<int>(byte) << " ";
            }
            RCLCPP_INFO(logger_,
                        "Joint frame len=%zu data_len=%u hex=[%s]",
                        frame.size(), data_len, frame_hex.str().c_str());
            RCLCPP_INFO(logger_,
                        "Joint raw: [%u, %u, %u, %u, %u, %u], "
                        "angles_deg: [%.2f, %.2f, %.2f, %.2f, %.2f, %.2f], "
                        "gripper=%.0f, run_status=0x%02X(%s)",
                        joint_raw_values[0], joint_raw_values[1], joint_raw_values[2],
                        joint_raw_values[3], joint_raw_values[4], joint_raw_values[5],
