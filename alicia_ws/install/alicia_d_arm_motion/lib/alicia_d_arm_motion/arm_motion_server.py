...
class ArmMotionServer(Node):
    ...
    def _execute_pose_move(self, goal_handle):
        ...
        try:
            ...
            success, error_code, detail = self._execute_trajectory(trajectory)
            if not success:
                return self._abort(
                    goal_handle,
                    result,
                    error_code,
                    f"Trajectory execution failed: {detail}",
                )
    ...
    def _execute_trajectory(self, trajectory):
        ...
        self.get_logger().info(f"Executing trajectory: {summary}")
        goal = ExecuteTrajectory.Goal()
        goal.trajectory = trajectory
        start_time = time.monotonic()
        send_future = self._execute_client.send_goal_async(goal)
        if not self._wait_future(send_future, 5.0):
            detail = f"timeout waiting for execute goal acceptance; {summary}"
            self.get_logger().error(detail)
            return False, 0, detail
        goal_handle = send_future.result()
        if not goal_handle.accepted:
            detail = f"execute goal rejected by /execute_trajectory; {summary}"
            self.get_logger().error(detail)
            return False, 0, detail
        ...
        if not self._wait_future(result_future, self.execution_timeout_s):
            ...
            detail = (
                "timeout waiting for execute result after "
                f"{elapsed_s:.3f}s; timeout={self.execution_timeout_s:.3f}s; {summary}"
            )
            self.get_logger().error(detail)
            return False, 0, detail
        ...
        if error_code != self.MOVEIT_SUCCESS:
            detail = (
                f"/execute_trajectory returned error_code={error_code} after "
                f"{elapsed_s:.3f}s; {summary}"
            )
            self.get_logger().error(detail)
            return False, error_code, detail
        self.get_logger().info(
            f"ExecuteTrajectory succeeded after {elapsed_s:.3f}s; {summary}"
        )
        return True, error_code, "execution succeeded"

    @staticmethod
    def _format_positions(positions):
        return "[" + ", ".join(f"{value:.6f}" for value in positions) + "]"

    def _summarize_trajectory(self, trajectory):
        joint_trajectory = trajectory.joint_trajectory
        names = list(joint_trajectory.joint_names)
        points = list(joint_trajectory.points)
        if not points:
            return f"joint_names={names}, points=0"
        ...
        first_positions = self._format_positions(first.positions)
        last_positions = self._format_positions(last.positions)
        return (
            f"joint_names={names}, points={len(points)}, duration={duration_s:.3f}s, "
            f"first={first_positions}, last={last_positions}"
        )
