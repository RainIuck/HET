...
class PickCoordinator(Node):
    ...
    def _declare_parameters(self):
        ...
        self.declare_parameter("place_path_file", "")
    ...
    def _load_parameters(self):
        ...
        self.place_path_file = str(self.get_parameter("place_path_file").value)
        self.place_path_enabled = False
        self.place_path_joint_names = []
        self.place_path_waypoints = []
        self.place_path_error = ""
        self._load_place_path()
    ...
    def _execute_candidate(self, candidate):
        try:
            ...
            if waypoint_error:
                ...
                return
            if self.place_path_error:
                self.get_logger().error(self.place_path_error)
                return
            ...
            if self.place_path_enabled:
                if not self._move_arm_through_joint_path(
                    "moving_place_path",
                    self.place_path_joint_names,
                    self.place_path_waypoints,
                ):
                    self.get_logger().error("Could not complete place path")
                    return
                if not self._move_gripper("releasing_object", self.gripper_open_ticks):
                    self.get_logger().error("Could not release object at place pose")
                    return
                return_waypoints = list(reversed(self.place_path_waypoints[:-1]))
                if return_waypoints and not self._move_arm_through_joint_path(
                    "returning_place_path",
                    self.place_path_joint_names,
                    return_waypoints,
                ):
                    self.get_logger().error("Could not return along place path")
                    return
                if not self._move_arm_to_joints(
                    "moving_wait_after_place",
                    self.post_grasp_intermediate_joint_names,
                    self.post_grasp_intermediate_joint_positions,
                ):
                    self.get_logger().error("Could not return to post-grasp intermediate waypoint")
                    return
    ...
    def _load_place_path(self):
        path = self.place_path_file.strip()
        if not path:
            self.get_logger().info("No place_path_file configured; place path disabled")
            return
        try:
            with open(path, "r", encoding="utf-8") as config_file:
                data = yaml.safe_load(config_file) or {}
        except Exception as exc:
            self.place_path_error = f"Could not read place path file '{path}': {exc}"
            return

        config = data.get("place_path", data)
        self.place_path_enabled = bool(config.get("enabled", False))
        if not self.place_path_enabled:
            self.get_logger().info(f"Place path disabled by config: {path}")
            return

        self.place_path_joint_names = [
            str(value) for value in config.get("joint_names", [])
        ]
        raw_waypoints = config.get("waypoints", [])
        if not isinstance(raw_waypoints, list):
            self.place_path_error = (
                f"Place path waypoints in {path} must be a list"
            )
            return
        self.place_path_waypoints = []
        for index, waypoint in enumerate(raw_waypoints):
            ...
            try:
                ...
                self.place_path_error = (
                    f"Place path waypoint {index} ('{name}') in {path} "
                    "contains a non-numeric joint position"
                )
                return
            self.place_path_waypoints.append(
                {
                    "name": name,
        ...
        self.place_path_error = self._validate_joint_waypoint_path(
            "Place path",
            self.place_path_joint_names,
            self.place_path_waypoints,
            path,
        ...
        if self.place_path_error:
            return
        self.get_logger().info(
            f"Loaded place path with {len(self.place_path_waypoints)} waypoint(s): {path}"
        )

    @staticmethod
    def _validate_joint_waypoint_path(label, joint_names, waypoints, source):
        expected_names = {"Joint1", "Joint2", "Joint3", "Joint4", "Joint5", "Joint6"}
        if (
            len(joint_names) != 6
            or len(set(joint_names)) != 6
            or set(joint_names) != expected_names
        ):
            return (
                f"{label} has invalid joint_names in {source}: expected one each of "
                f"{sorted(expected_names)}, got {joint_names}"
            )
        if not waypoints:
            return f"{label} is enabled but has no waypoints in {source}"
        for index, waypoint in enumerate(waypoints):
            positions = waypoint.get("positions", [])
            if len(positions) != 6:
                return (
                    f"{label} waypoint {index} ('{waypoint.get('name', '')}') in "
                    f"{source} must contain 6 joint positions"
                )
            if not all(math.isfinite(value) for value in positions):
                return (
                    f"{label} waypoint {index} ('{waypoint.get('name', '')}') in "
                    f"{source} contains non-finite joint positions"
                )
        return ""
    ...
    def _move_arm_through_joint_path(self, stage_prefix, joint_names, waypoints):
        total = len(waypoints)
        for index, waypoint in enumerate(waypoints):
            stage = f"{stage_prefix}_{index + 1}_of_{total}_{waypoint['name']}"
            if not self._move_arm_to_joints(stage, joint_names, waypoint["positions"]):
                return False
        return True
