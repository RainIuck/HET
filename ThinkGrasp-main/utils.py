...
def get_true_heightmap(env):
    ...
    imageio.imwrite('color_map.png', cmap)

    # Height maps and masks are single-channel images, save them in a format that supports grayscale images without applying a colormap.
    # For the height map, you may want to normalize it first depending on its range.
    normalized_hmap = (hmap - hmap.min()) / (hmap.max() - hmap.min()) * 255
    imageio.imwrite('height_map.png', np.uint8(normalized_hmap))
...
def convert_outputnew(image_pil, boxes, logits, phrases, color_image, depth_image,
    ...
    for box, logit, phrase in zip(boxes, logits, phrases):
        # Border box coordinates conversion to integers
        ...
        if preferred_grasping_location < 1 or preferred_grasping_location > 9:
            preferred_grasping_location = 5  # Default to center if the provided location is invalid

        row, col = grid_cells[preferred_grasping_location - 1]
        ...
        # Crop image to the preferred grid cell
...
def convert_output(image_pil, boxes, logits, phrases, color_image, depth_image, mask_image,
                   preferred_grasping_location):
    bbox_images = []
    bbox_positions = []
    bbox_depthimages = []
...
def visualize_data(bbox_images, bboxes, pos_bboxes, grasps):
    fig, axs = plt.subplots(1, len(bbox_images), figsize=(20, 5))
    for i, img in enumerate(bbox_images):
        if len(bbox_images) == 1:
            ax = axs
        else:
            ax = axs[i]
        ax.imshow(img)
    ...
    if pos_bboxes is not None:
        ...
        for i in range(pos_bboxes.size(1)):
            img = F.to_pil_image(pos_bboxes[0, i])
            if pos_bboxes.size(1) == 1:
                ax = axs
            else:
                ax = axs[i]
            ax.imshow(img)
...
def visualize_processed_data(remain_bbox_images, bboxes, pos_bboxes, grasps):
    if bboxes is not None and bboxes.nelement() > 0:
        grid_img = make_grid(bboxes.squeeze(0), nrow=5)  
    ...
    if pos_bboxes is not None and pos_bboxes.nelement() > 0:
        ...
        for pos in pos_bboxes.squeeze(0):
            plt.plot(pos[0].item(), pos[1].item(), 'ro')  
        plt.title("BBox Positions")
...
def get_and_process_data(cropping_box,color, depth):
    ...
    cloud_sampled = cloud_sampled.to(torch.device('cuda' if torch.cuda.is_available() else 'cpu'))  # 假设使用GPU
